...
def _compose_grasp_orientation(base_rotation: RotationMatrix, angle_rad: float) -> RotationMatrix:
    """将基础朝向与绕Z轴旋转角组合为最终抓取朝向。"""
    return base_rotation.multiply(RotationMatrix.MakeZRotation(angle_rad))
...
class CandidateEvaluator:
    ...
    def _compute_base_rotation(self, template: GraspTemplate, object_center_x: float | None = None) -> RotationMatrix:
        """计算模板指定的基础朝向（考虑动态俯仰角）。"""
        ...
        return RollPitchYaw(*xyz_rad).ToRotationMatrix()
